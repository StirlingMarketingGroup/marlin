...
export default function AppIcon({
  ...
  return (
    <div ref={ref} className={className}>
      ...
      {(!dataUrl || !loaded) && (
        <div
          className={`${rounded ? 'rounded' : ''} w-full h-full bg-app-gray/50 ${loading ? 'animate-pulse' : ''} flex items-center justify-center`}
        >
          {fallback || null}
        </div>
      )}
      ...
      {dataUrl && shouldShowThumbnail ? (
        <img
          ...
          title={
            error
              ? `Error: ${error}`
              : `${cached ? 'Cached' : 'Generated'} in ${generationTimeMs}ms`
          }
        />
      ) : null}
