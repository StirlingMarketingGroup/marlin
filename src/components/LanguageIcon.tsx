...
export function LanguageIcon({ label, bg, color, size = 'small', className = '', title }: LanguageIconProps) {
  ...
  return (
    <div
      ...
      style={{ backgroundColor: bg, color: color || pickTextColor(bg), width: size === 'small' ? 20 : 48, height: size === 'small' ? 20 : 48 }}
      title={title || label}
      aria-hidden
    >
      <span className={`${textClass} leading-none select-none`} style={{ transform: 'translateY(0.5px)' }}>{label}</span>
    </div>
  )
}
...
export function resolveLangIcon(name: string, ext?: string): LangMeta | undefined {
  ...
  if (filename === 'dockerfile' || filename.startsWith('dockerfile')) return { label: 'DK', bg: '#0db7ed' }
  ...
  if (filename === 'pyproject.toml' || filename === 'requirements.txt') return { label: 'PY', bg: '#3776AB' }
