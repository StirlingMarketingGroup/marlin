...
export default function DragDebug() {
  ...
  return (
    <div
      className="fixed bottom-4 right-4 bg-app-dark border border-app-border rounded-lg p-4 shadow-lg"
    ...
    >
      ...
      <div className="mt-3 text-xs">
        ...
        {dragEvents.map((event, i) => (
          <div key={i} className="text-app-text font-mono text-[10px]">
            {event}
          </div>
        ))}
        {dragEvents.length === 0 && <div className="text-app-muted">No events yet...</div>}
      </div>
    </div>
  );
}
