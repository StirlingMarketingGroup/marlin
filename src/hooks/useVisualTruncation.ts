...
// Average character width approximation (in pixels, for common UI fonts)
...
export function useVisualTruncation(
  ...
  const cacheKey = useMemo(() => {
    ...
    return `${text}-${width}`
  }, [text, maxWidth, containerRef.current?.offsetWidth])

  // Get computed font size for better character width estimation
  const getFontSize = useCallback((): number => {
    if (!containerRef.current) return 14
    const computedStyle = window.getComputedStyle(containerRef.current)
    return parseFloat(computedStyle.fontSize) || 14
  }, [containerRef])
  ...
  }, [text, cacheKey, enabled, maxWidth, containerRef, getFontSize])
