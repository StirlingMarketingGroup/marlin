...
function cacheKey(text: string, width: number, font: FontProps, preserveExtension: boolean): string {
  return [text, width, font.fontSize, font.fontFamily, font.fontWeight, font.fontStyle || 'normal', font.lineHeightPx, preserveExtension ? '1' : '0'].join('|')
}
...
export function truncateToTwoLines(
  ...
  if (best === ellipsis + extension && hasExtension) {
    ...
    for (let i = extension.length; i >= 1; i--) {
      ...
      if (lineCountOf(candidate, width, font) <= maxLines) { best = candidate; break }
    }
  }
