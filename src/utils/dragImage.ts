...
type DragVisual = {
  file: FileItem;
  el?: HTMLElement | null;
  prerenderedIcon?: HTMLImageElement | null;
};
...
function drawFolderIcon(ctx: CanvasRenderingContext2D, x: number, y: number, w: number, h: number) {
  ...
  ctx.arcTo(
    x + w,
    y + h - folderHeight + tabHeight,
    x + w,
    y + h - folderHeight + tabHeight + cornerRadius,
    cornerRadius
  );
...
function drawThumbOrBadgeWithIcon(
  ctx: CanvasRenderingContext2D,
  vis: DragVisual,
  x: number,
  y: number,
...
export async function createDragImageForSelectionAsync(
  ...
  visuals.forEach((vis, i) => {
    vis.prerenderedIcon = prerenderedIcons[i];
  });
  ...
  for (let i = count - 1; i >= 0; i--) {
    const dx = spread * i;
    const dy = spread * i;
    drawThumbOrBadgeWithIcon(ctx, visuals[i], dx, dy, base, visuals[i].prerenderedIcon);
  }
  ctx.restore();
