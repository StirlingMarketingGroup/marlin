...
export function measureText(text: string, fontSize: number = 13, fontFamily: string = 'system-ui'): number {
...
export function truncateTextToWidth(
  ...
  const fullWidth = measureText(text, fontSize, fontFamily)
  ...
  if (preserveExtension) {
    ...
    if (hasExtension) {
      ...
      const extensionWidth = measureText(extension, fontSize, fontFamily)
      const baseName = text.substring(0, lastDotIndex)
      
      const availableWidth = maxWidth - extensionWidth - ellipsisWidth
      
      if (availableWidth <= 0) {
        const truncated = truncateFromEnd(text, maxWidth - ellipsisWidth, fontSize, fontFamily)
        return { text: truncated + ellipsis, isTruncated: true }
      }
      
      const halfWidth = availableWidth / 2
      
      const startPart = truncateFromStart(baseName, halfWidth, fontSize, fontFamily)
      const endPart = truncateFromEnd(baseName, halfWidth, fontSize, fontFamily, baseName.length - startPart.length)
  ...
  const startPart = truncateFromStart(text, halfWidth, fontSize, fontFamily)
  const endPart = truncateFromEnd(text, halfWidth, fontSize, fontFamily, text.length - startPart.length)
...
function truncateFromStart(
  ...
  while (low <= high) {
    ...
    const width = measureText(substr, fontSize, fontFamily)
