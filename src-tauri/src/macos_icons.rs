...
pub fn app_icon_png_base64(path: &str, size: u32) -> Result<String, String> {
  unsafe {
    ...
    if workspace == nil { return Err("NSWorkspace unavailable".into()); }
    let image: id = msg_send![workspace, iconForFile: ns_path];
    if image == nil { return Err("iconForFile returned nil".into()); }
    ...
    if tiff == nil { return Err("TIFFRepresentation is nil".into()); }
    let rep_class = class!(NSBitmapImageRep);
    let rep: id = msg_send![rep_class, imageRepWithData: tiff];
    if rep == nil { return Err("imageRepWithData returned nil".into()); }
    ...
    if png_data == nil { return Err("PNG conversion failed".into()); }
    ...
    if bytes_ptr.is_null() || len == 0 { return Err("PNG data is empty".into()); }
