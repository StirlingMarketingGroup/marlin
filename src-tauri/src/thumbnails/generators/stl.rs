...
impl Vec3 {
    ...
    fn normalize(self) -> Self { let l = self.len(); if l > 0.0 { self.mul(1.0/l) } else { self } }
}

#[derive(Clone, Copy, Debug)]
struct Tri { v0: Vec3, v1: Vec3, v2: Vec3 }
...
impl StlGenerator {
    pub fn generate(request: &ThumbnailRequest) -> Result<String, String> {
        ...
        if tris.is_empty() { return Err("No triangles in STL".into()); }
        ...
        for t in &tris {
            for v in [t.v0, t.v1, t.v2] {
                ...
                min2.x = min2.x.min(p.x); min2.y = min2.y.min(p.y); min2.z = min2.z.min(p.z);
                max2.x = max2.x.max(p.x); max2.y = max2.y.max(p.y); max2.z = max2.z.max(p.z);
            }
        }
        ...
        for t in &tris {
            ...
            let minx = sx.iter().cloned().fold(f32::INFINITY, f32::min).floor().max(0.0) as i32;
            let maxx = sx.iter().cloned().fold(f32::NEG_INFINITY, f32::max).ceil().min((target-1) as f32) as i32;
            let miny = sy.iter().cloned().fold(f32::INFINITY, f32::min).floor().max(0.0) as i32;
            let maxy = sy.iter().cloned().fold(f32::NEG_INFINITY, f32::max).ceil().min((target-1) as f32) as i32;
            if minx > maxx || miny > maxy { continue; }

            // Precompute edge function coefficients
            let e = |x0:f32,y0:f32,x1:f32,y1:f32,x:f32,y:f32| -> f32 { (x - x0)*(y1 - y0) - (y - y0)*(x1 - x0) };
...
fn parse_stl(path: &Path) -> Result<Vec<Tri>, String> {
    ...
    let mut buf = Vec::new();
    f.read_to_end(&mut buf).map_err(|e| format!("Failed to read STL: {}", e))?;
...
fn parse_binary_stl(data: &[u8], tri_count: usize) -> Result<Vec<Tri>, String> {
    ...
    for _ in 0..tri_count {
        if i + 50 > data.len() { break; }
        let n = read_vec3_le(&data[i..]); i += 12;
        let v0 = read_vec3_le(&data[i..]); i += 12;
        let v1 = read_vec3_le(&data[i..]); i += 12;
        let v2 = read_vec3_le(&data[i..]); i += 12;
        ...
        let _nrm = {
            ...
            if nn.len() == 0.0 { nn = (v1.sub(v0)).cross(v2.sub(v0)).normalize(); }
            nn
        };
        tris.push(Tri { v0, v1, v2 });
        if tris.len() >= 500_000 { break; } // hard safety cap
    }
    Ok(tris)
}
    ...
    for line in reader.lines() {
        ...
        if t.starts_with("facet normal") {
            ...
            if verts.len() >= 3 {
                ...
                if n.len() == 0.0 { n = (v1.sub(v0)).cross(v2.sub(v0)).normalize(); }
                let _ = n; // computed for consistency; not stored
                tris.push(Tri { v0, v1, v2 });
            }
            verts.clear();
        }
        if tris.len() >= 500_000 { break; }
    }
